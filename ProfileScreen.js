...
export default function ProfileScreen() {
  ...
  {isGuest ? (
    ...
    <>
      ...
      <Text style={styles.label}>SOCIALLINKS</Text>
      <TextInput 
        style={styles.input} 
      ...
      <TextInput 
        ...
        editable={false} // Username shouldn't usually change
      />

      {/* ... Add the rest of your fields (Hobbies, Gender, Phone) here ... */}
...
// Helper component to keep code clean and resilient
const DetailItem = ({ icon, label, value }) => (
  <View style={styles.detailRow}>
    <Ionicons name={icon} size={20} color="#888" />
    <Text style={styles.detailText}>{label}: {value || 'Not set'}</Text>
  </View>
);
...
const styles = StyleSheet.create({
  ...
  detailText: { color: '#fff', fontSize: 16, marginLeft: 15 },
  centerBox: { alignItems: 'center', padding: 20 },
  guestWarning: { color: '#aaa', textAlign: 'center', marginTop: 10, fontStyle: 'italic' },
