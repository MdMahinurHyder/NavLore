...
export default function HomeScreen() {
  ...
  // NEW: State to track if a search failed
  ...
  const handleSearch = () => {
    ...
    if (found) {
      setNotFound(false); // Clear any errors
      router.push({ pathname: '/placeDetails', params: { id: found.id } });
    } else {
      // Show our custom UI instead of an Alert
      setNotFound(true); 
    }
  };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.searchBox}>
        ...
        <TextInput 
          ...
          style={styles.input}
          value={search}
          // Clear the "Not Found" screen immediately if the user starts typing again
          onChangeText={(text) => {
            setSearch(text);
        ...
        />
        {/* Little 'X' button inside the search bar if there is text */}
        {search.length > 0 && (
      ...
      {/* CONDITIONAL RENDER: Show Error UI OR Grid */}
      {notFound ? (
